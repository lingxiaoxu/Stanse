 ...
 * 结构校验：检查问题格式是否正确
 */
export function validateStructure(question: GeneratedQuestion): ValidationResult {
  const errors: string[] = [];
  ...
  const validCategories: QuestionCategory[] = ['FLAGS', 'LANDMARKS', 'ANIMALS', 'FOOD', 'LOGOS', 'SYMBOLS'];
 ...
 * 校验原始 JSON 格式的问题
 */
export function validateRawQuestion(raw: {
  id: string;
  stem: string;
  category: string;
  difficulty: string;
...
): Promise<AgentResult<ValidatedQuestion>> {
  const logs: string[] = [];
 ...
 * 批量校验 JSON 文件中的原始问题
 */
export async function batchValidateRawQuestions(
  rawQuestions: Array<{
    id: string;
    stem: string;
    category: string;
